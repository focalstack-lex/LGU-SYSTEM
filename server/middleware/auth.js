...
// FORENSIC TRACKER watch list - configurable strictly via env (comma-separated).
// Logs carry request METADATA only (IP, user agent, method, path) - never request bodies.
const WATCH_EMAILS = (process.env.FORENSIC_WATCH_EMAILS || '')
  .split(',').map((e) => e.trim().toLowerCase()).filter(Boolean);
const WATCH_IDS = (process.env.FORENSIC_WATCH_IDS || '')
...
module.exports = async function authMiddleware(req, res, next) {
  ...
  const isWatched = (WATCH_EMAILS.length > 0 && WATCH_EMAILS.includes(String(user.email || '').toLowerCase())) ||
                    (WATCH_IDS.length > 0 && WATCH_IDS.includes(user.id));
  if (isWatched) {
